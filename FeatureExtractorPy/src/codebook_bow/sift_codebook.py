...
from databasehandler import imagesHandler, sift_cb_handler
...
def extract_all_descriptors(resize_ratio, resize_method, crop_amount):
    ...
    sift_cb_handler.store_descriptors(concatDesc)
    return concatDesc
...
def performKmeans(data,n_clusters):
    ...
    est.fit(data)
    sift_cb_handler.store_estimator(est)
...
def build_distribution(est, n_clusters, resize_ratio, resize_method, crop_amount):
    ...
    sift_cb_handler.store_distributions(allDist)
    
    
def run_codebook(n_clusters,resize_ratio, resize_method, crop_amount):
    sift_cb_handler.drop_database_table()
    sift_cb_handler.create_database_table()
    desc = extract_all_descriptors( resize_ratio, resize_method, crop_amount)
    est = performKmeans(desc,n_clusters)
    build_distribution(est, n_clusters, resize_ratio, resize_method, crop_amount)
